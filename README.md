...
> Local alpha `0.8.1` (см. CHANGELOG). Запускайте только на `127.0.0.1` и направляйте
> `WORKSPACE_DIR` на отдельную рабочую копию. Изменения файлов от агентов
> требуют ручного подтверждения в UI. Vite `dist/` primary, legacy `/static`/`/chat` удалены в 0.8.1.
...
|---------------|------------------------------------------------------|----------------------------------------|
| PlannerAgent  | OpenRouter: `meta-llama/llama-3-70b-instruct:free`   | Стратегическое планирование           |
| CriticAgent   | OpenRouter: `meta-llama/llama-3-70b-instruct:free`   | Ревью и отбраковка планов              |
| ExecutorAgent | OpenRouter: `google/gemma-2-9b-it:free`              | Исполнение кода и работа с файлами    |
