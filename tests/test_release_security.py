...
def test_dangerous_tools_are_not_exposed(temp_workspace: Path) -> None:
    ...
    # delete_file теперь с approval (0.6.0) — считается безопасным, как write_file
    assert {"execute_bash", "execute_git"}.isdisjoint(names)
    assert {"read_file", "write_file", "replace_in_file", "delete_file", "search_in_file", "list_dir"} <= names
