...
> Local alpha `1.0.0` (см. CHANGELOG). Запускайте только на `127.0.0.1` и направляйте
> `WORKSPACE_DIR` на отдельную рабочую копию. Изменения файлов от агентов
> требуют ручного подтверждения в UI. Vite `dist/` primary, legacy `/static`/`/chat` удалены в 1.0.0.
