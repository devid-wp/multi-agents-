...
async def test_post_settings_openrouter_provider_accepted(app_client: httpx.AsyncClient) -> None:
    """
    OpenRouter — основной провайдер Trinity. POST с provider=openrouter
    должен приниматься валидатором AgentProviderConfig и сохраняться
    в cookie-сессии.
    """
    r = await app_client.post(
        "/api/settings",
        json={
            "planner": {
                "provider": "openrouter",
                "api_key": "sk-or-v1-OPENTEST1234567890abcdef",
    ...
    body = r.json()
    assert body["ok"] is True
    assert body["planner"]["provider"] == "openrouter"
    assert body["critic"]["provider"] == "openrouter"
    assert body["executor"]["provider"] == "openrouter"
    assert body["planner"]["has_key"] is True
    assert body["planner"]["model_name"] == "meta-llama/llama-3-70b-instruct:free"
    assert body["executor"]["model_name"] == "google/gemma-2-9b-it:free"
    # Маскирование ключа для UI
    assert "***" in (body["planner"]["key_masked"] or "")
