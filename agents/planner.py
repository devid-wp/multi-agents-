...
6. Учитывай, что Executor работает с инструментами:
   ...
   - read_file(путь, start_line, end_line)
   - write_file(путь, содержимое)
   - replace_in_file(path, target_content, replacement_content)
   - delete_file(путь)
   - search_in_file(путь, pattern)
   - list_dir(путь)
...
9. ВАЖНО: Последним шагом плана ВСЕГДА должна быть верификация написанного или измененного кода (например, запуск тестов, линтера, скрипта или команды для проверки результата). Никаких завершений без проверки!
